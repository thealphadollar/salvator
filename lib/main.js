...
async function main() {
    ...
    const { links, names } = await getBirthdayData(browser, fbID, fbPass);

    /* Displaying friends who have birthday today */
    let count = 1;
    names.fullNames = names.fullNames.forEach(name => {
    ...
    if (newLinks.length === 0) {
        console.log("No Birthdays found. Exiting...");
        await browser.close();
        return;
    }
