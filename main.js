...
async function initMessages(
  ...
  for (let i = 0; i < messageLinks.length; i += 1) {
    let message = '';
    if (prevLinks.indexOf(messageLinks[i]) > -1) continue;
    const ranWish = genWish();
    if (firstNames) {
...
async function checkHistory() {
  ...
  for (let i = 0; i < obj.links.length; i += 1) array[i] = obj.links[i];
...
async function writeHistory(messageLinks, prevLinks) {
  ...
  for (let i = 0; i < prevLinks.length; i += 1) {
    if (list.indexOf(prevLinks[i]) > -1) continue;
